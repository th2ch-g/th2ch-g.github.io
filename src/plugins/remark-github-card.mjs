...
// into a Twitter-style social preview card. Owner / repo / description
// come from the public GitHub REST API; the hero image is GitHub's own
// auto-generated social preview (the same image Twitter / Slack render).
// Both are fetched at build time and cached on disk so subsequent builds
// don't re-hit the 60-req/h unauthenticated API rate limit. Network /
// API failures fall back to a stale cache when available, then to no-op
// (the URL is left alone and renders as a normal Markdown link) — never
// blocks the build.
//
// Output uses the same `.link-card-*` CSS classes as remark-link-card so
// every pasted URL — GitHub or otherwise — renders with the same
// summary_large_image layout (full-width image on top, title / site at
// the bottom). The split between the two plugins is kept because GitHub
...
async function fetchRepo(owner, repo) {
  ...
  const ownerLogin = json.owner?.login ?? owner;
  const name = json.name ?? repo;
  const fullName = json.full_name ?? `${ownerLogin}/${name}`;
  const description = json.description ?? '';
  return {
    // Mirrors the <title> shape GitHub itself ships ("GitHub - owner/repo: desc")
    // so a pasted GitHub URL on this site reads the same as one expanded by
    // Twitter / Slack.
    title: description ? `GitHub - ${fullName}: ${description}` : `GitHub - ${fullName}`,
    description,
    url: json.html_url ?? `https://github.com/${owner}/${repo}`,
    // GitHub auto-generates a rich social preview here. The first path
