...
| **Boltz-Perturb** (TBP/TCP) | 2 | H. Jung, B. Lee, A. C. Cheng / Merck & Co. | タンパク質-リガンドco-folding(Boltz-2)でリガンド結合ポーズの多様性と精度を改善 | denoising moduleが受け取るconditioning信号を推論時に摂動。single表現 si を摂動するTCPとpair表現由来のattention bias B を摂動するTBPの2種、リガンドtokenに限定し局所標準偏差でスケール、拡散ステップ依存のノイズスケジュール(TADS / TADS-Auto)で強度を調整 | 再学習不要、oracle成功率 17.7%→30.6%(62標的)、高温サンプリング比で約78%少ない計算量、confidence上位20でも最良ベースライン比で成功率2倍超(5.26%→14.04%)、正しいポーズは既にモデル内に存在するという診断実験(真座標注入)付き | リガンド結合ポーズが主眼でタンパク質のコンフォメーション変化ではない、Boltz-2特化、訓練セットとの類似度が最も低い領域では改善なし、confidenceが精度と乖離、TBPはメモリ負荷が大きくV100 32GBで一部評価不能、プレプリント | 10.64898/2026.08.05.742877 (bioRxiv) |
