...
- `orcid-cv-sync` — pull new publications from ORCID into `src/content/cv/{ja,en}.md`. Never rewrites an existing entry; its one deletion rule drops a preprint once its peer-reviewed version is listed in the same file (CrossRef `is-preprint-of`, falling back to title match). Trigger when refreshing the publications list.
