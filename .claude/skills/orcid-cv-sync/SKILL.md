---
name: orcid-cv-sync
description: Sync newly published peer-reviewed papers, preprints, and funding records from the ORCID Public API into the bilingual CV (src/content/cv/ja.md and en.md), and drop preprints that have since appeared as peer-reviewed papers. Use whenever the user wants to refresh their publications or funding list, pull from ORCID, "ORCIDから取り込み", "CV更新", "論文一覧をアップデート", "FundingをCVに記載", "研究助成を同期", "publications を同期", "新しい論文を CV に入れて", "プレプリントが査読付きで出た", or whenever an ORCID work or funding record has changed and the CV is likely stale. The skill adds new entries and never rewrites existing ones; its single deletion rule removes a preprint entry once its peer-reviewed version is listed in the CV. Both ja.md and en.md receive the same updates.
---

# Sync publications and funding from ORCID into the CV

This portfolio's CV pages (`src/content/cv/ja.md` and `src/content/cv/en.md`) carry hand-written publication and funding sections. ORCID is the source of truth for *new* journal articles, preprints, and funding records; this skill picks up entries from ORCID that aren't already in either file and proposes additions.
...
9. Extracts one entry per ORCID funding group, preferring a trimmed `grant_number` external ID as its stable identity and falling back to the ORCID put-code. Each locale keeps an independent known-key set. Existing entries are never rewritten; a visible grant number without a script marker also counts as known so hand-written entries are not duplicated.
10. Sorts new funding by start date descending and inserts localized Markdown lines into the `funding` marker. A matching ORCID translated title is used when present; unavailable organization, URL, ID, and date components are omitted rather than inferred. Amount and funding type are not emitted because they are absent from the summary response or unsuitable as CV labels.
11. **Prunes superseded preprints.** Walks the `preprints` section of each file and removes any entry whose peer-reviewed version is listed in *that same file's* `peer-reviewed` section. Two signals, in order of trust:
...
Funding entries are unordered list items. The stable identity is stored in a trailing inline comment so repeat runs are idempotent without exposing implementation metadata in rendered HTML:

```markdown
- [JST Science Tokyo BOOST](https://www.tmd.ac.jp/campuslife/boost/), JST, Grant No. JPMJBS2430, April 2025–March 2028. <!-- orcid-funding:grant-number:jpmjbs2430 -->
```
...
- **Funding record has no grant number**: the script uses the ORCID put-code as its hidden stable identity. It still renders the available title, organization, URL, and date fields.
- **Funding entry was written by hand**: a matching visible external ID suppresses an automated duplicate even without an `orcid-funding` marker. A title-only match is added with a trailing `POSSIBLE DUPLICATE` warning for review.
...
- **Never modify an existing entry, and delete only superseded preprints.** Additions are the default; the sole deletion is a preprint whose peer-reviewed version is already listed in the same file. Nothing in `funding`, `peer-reviewed`, or `presentations` is ever removed, and no entry's text is ever rewritten in place. Deletions always appear in the dry-run diff first.
...
- Commit the changes with a message like `Sync CV data from ORCID`.
